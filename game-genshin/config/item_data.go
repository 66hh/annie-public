...
func (g *GameDataConfig) loadItemData() {
	...
	for _, itemData := range g.ItemDataMap {
		itemData.ItemEnumType = itemTypeConst.STRING_MAP[itemData.ItemType]
		itemData.MaterialEnumType = materialTypeConst.STRING_MAP[itemData.MaterialType]

		if itemData.ItemEnumType == itemTypeConst.ITEM_RELIQUARY {
			itemData.EquipEnumType = equipTypeConst.STRING_MAP[itemData.EquipType]
		...
		} else if itemData.ItemEnumType == itemTypeConst.ITEM_WEAPON {
			itemData.EquipEnumType = equipTypeConst.EQUIP_WEAPON
		} else {
			itemData.EquipEnumType = equipTypeConst.EQUIP_NONE
		}

		if itemData.WeaponProp != nil {
			fightPropertyConst := constant.GetFightPropertyConst()
			for i, v := range itemData.WeaponProp {
				v.FightProp = fightPropertyConst.STRING_MAP[v.PropType]
