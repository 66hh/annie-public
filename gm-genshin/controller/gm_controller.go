...
func (c *Controller) changeGateState(context *gin.Context) {
	...
	var res bool
	ok := c.rpcGenshinGatewayConsumer.CallFunction("RpcManager", "ChangeGateOpenState", &state, &res)
	if ok == true && res == true {
...
func (c *Controller) kickPlayer(context *gin.Context) {
	...
	var result bool
	ok := c.rpcGenshinGatewayConsumer.CallFunction("RpcManager", "KickPlayer", &info, &result)
...
func (c *Controller) getOnlineUser(context *gin.Context) {
	...
	userId := uint32(uid)
	ok := c.rpcGenshinGatewayConsumer.CallFunction("RpcManager", "GetOnlineUser", &userId, &list)
	if ok {
...
func (c *Controller) forbidUser(context *gin.Context) {
	...
	var result bool
	ok := c.rpcGenshinGatewayConsumer.CallFunction("RpcManager", "ForbidUser", &info, &result)
	if ok == true && result == true {
		context.JSON(http.StatusOK, dto.NewResponseResult(0, "操作成功", nil))
	} else {
		context.JSON(http.StatusOK, dto.NewResponseResult(-1, "操作失败", nil))
...
func (c *Controller) unForbidUser(context *gin.Context) {
	...
	userId := uint32(uid)
	var result bool
	ok := c.rpcGenshinGatewayConsumer.CallFunction("RpcManager", "UnForbidUser", &userId, &result)
	if ok == true && result == true {
		context.JSON(http.StatusOK, dto.NewResponseResult(0, "操作成功", nil))
	} else {
		context.JSON(http.StatusOK, dto.NewResponseResult(-1, "操作失败", nil))
