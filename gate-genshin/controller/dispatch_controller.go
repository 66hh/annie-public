...
func (c *Controller) query_security_file(context *gin.Context) {
	...
	if err != nil {
		c.log.Error("open security_file error")
		return
	}
...
func (c *Controller) query_cur_region(context *gin.Context) {
	...
	if strings.Contains(versionName, "2.7.5") || strings.Contains(versionName, "2.8.") {
		...
		if keyId == "3" {
			...
			// 国服
			c.log.Error("current region enc key not exist")
			return
		}
		regionInfo, err := base64.StdEncoding.DecodeString(response)
		if err != nil {
			c.log.Error("decode region info error: %v", err)
			return
		}
		chunkSize := 256 - 11
		...
		for i := 0; i < numChunks; i++ {
			...
			if err != nil {
				c.log.Error("rsa enc error: %v", err)
				return
			}
			decrypt, err := endec.RsaDecrypt(encrypt, privKey)
			if err != nil {
				c.log.Error("rsa dec error: %v", err)
				return
			}
			if bytes.Compare(decrypt, chunk) != 0 {
				c.log.Error("rsa dec test fail")
				return
			}
		...
		if err != nil {
			c.log.Error("rsa sign error: %v", err)
			return
		}
		ok, err := endec.RsaVerify(regionInfo, signData, &signPrivkey.PublicKey)
		if err != nil {
			c.log.Error("rsa verify error: %v", err)
			return
		}
		if !ok {
			c.log.Error("rsa verify test fail")
			return
		}
		rsp := &api.QueryCurRegionRspJson{
